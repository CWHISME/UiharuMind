...
对话页里的二分类：普通对话（`Chat`，装扮演与工具人两档）与智能体（`Agent`）。由左栏切换器**唯一**决定，
列表归属、中间会话区、右栏面板全部跟随它；右栏不自持类型状态。

⚠️ 不要与 `ECharacterKind` 混淆：那是角色的档位（四档），这是**会话层面**的二分类。
...
「哪个档走哪条装配、归到哪一类会话」的**唯一定义处**：`IsAgent()`、`IsChat()`、`CanStartSession()`。
对话页已合并为单页，档位不再决定「去哪一页」，只决定「属于普通对话还是智能体」
