...
装配前等托管中的 server 连上，上限 10 秒（`McpManager.WarmupAsync`）。
不等的话第一次装配必然赶不上——`Resolve` 撞见还没取到工具的 server 只能跳过它、在后台起连接，
症状不是「慢一点」而是**第一轮模型没有那些工具，而用户不知道为什么**。
工具取回后常驻缓存，所以这笔等待通常每个进程只付一次（`Reload` 会清掉全部连接，之后要重付）。

**不在应用启动时预连**，也不在打开智能体页时——那一页就是启动默认页，等价于前者。
这个应用还有截图、剪贴板、快捷问答一堆与 MCP 无关的功能，不用智能体的人不该付这份钱。

⚠️ **预连与装配共用同一份禁用名单**（`McpManager.IsInPlay`）。这一轮注定挂不上的 server，
既不该为它拉起子进程，也不该为它等。曾经预连只看托管开关，于是角色明明禁掉的 server
照样等满超时，工具取回后再被 `Resolve` 原样丢掉。

⚠️ 「server 正在连」是**进程级**事实（连接与缓存都在单例上），「我这一轮在等它」是**会话级**的。
要显示在输入框旁边的是后者，见下方「忙碌原因」。
...
这个数**不只给面板看**：压缩要从输入预算里扣掉它才得出历史额度，交接文档水位要拿它凑出有效占用。
因此它缓在 `AgentHandle.Capabilities` 上惰性算一次——含系统提示的分词，
接进压缩之后就落在发消息路径上了，每轮现算不能接受。见上方「上下文预算」与 ADR 0009。
...
目前两档：`ConnectingMcp`（预连）与 `Compacting`（整理交接文档）。

两个来源、一处显示：交接文档在 `TurnDriver.Busy`，预连在 `ICharacterRunner.Busy`（等待发生在装配内部），
由渲染方合并成 `ConversationViewModel.BusyLabel`——输入区旁边只有一块提示。

⚠️ **一个枚举，不是几个并列的 bool。** 这些状态天然互斥、显示在同一处、共用同一套形状；
摊成 bool 的结果是每加一种就往 axaml 里复制一块 StackPanel，而且没有任何机制阻止两个同时为真。

⚠️ **作用域是会话级。** 曾经预连读的是 `McpManager` 上一个全局 bool，于是后台定时任务触发的预连
会点亮一个跟 MCP 毫无关系的扮演会话的提示，两条链路并发时先结束的那个还会把另一个的提示掐灭。
现在 `WarmupAsync` 收一个逐次调用的回调，全局标志与全局事件已退役。

⚠️ **Core 侧不带文案**，与 `TurnNotice` 同一条规矩。枚举 → 本地化键的映射只在 `BusyLabel` 一处，
有测试盯着「每一档都有话说」（漏一档的症状是提示块空着一行图标转圈）。
...
| **固定开销** | 系统提示 + 工具定义，每轮完整重发的那一坨 | `AgentCapabilitySnapshot.EstimatedTokens`，缓在 `AgentHandle.Capabilities` 上 |
| **历史额度** | 输入预算 − 固定开销，即历史消息真正能占的那部分 | `HistoryCompaction.HistoryQuotaFor` |
| **报告占用** | 服务端 usage 里最近一次响应的输入 token | `TurnUsageLedger.LastInput` |
| **有效占用** | `max(报告占用, 固定开销 + 历史估算)` | `TurnUsageLedger.EffectiveInput` |

谁比谁，是这套词唯一容易搞错的地方：

| 判定 | 分子 | 分母 |
|---|---|---|
| 折叠 0.5 / 截断 0.9 | 历史估算（`CorrectedTokenCount`，只数消息） | **历史额度** |
| 交接文档 0.8 | **有效占用** | 输入预算 |
| 进度条 | 两段：报告占用 + 未计入差额，合计即有效占用 | 上下文上限 |

⚠️ **「还剩多少空间」一律用有效占用，「花了多少钱」才用报告占用。**
服务端报的数**不一定是全量**——实测 GLM4-Flash 的 `prompt_tokens` 不含工具定义，少报近一半。
只信它的话交接文档水位永远不触发。两者的比值越界（0.8~1.2 之外）会打一条 Debug，
那是「服务端少报」或「我们的分词偏了」的唯一信号。

⚠️ **折叠与截断的分母必须扣掉固定开销。** 它们的分子只数消息，而系统提示与工具定义
不是消息、进不了 `CompactionMessageIndex`，却实打实吃掉分母。曾经乘在输入预算上，
于是一套 21k 的 MCP 工具集能让截断在请求**早已超出上下文之后**才动手。

⚠️ **「占用」不是 `TurnUsageLedger.TurnInput`。** 后者是本轮所有调用的累加（成本视角），
一轮 agent 十几次工具往返能累到四十几万，与「现在多满」无关。

界面上进度条的**整条是上下文上限**（用户能跟官方文档对上的那个数），
而水位是按预算/额度算的——所以 tooltip 给的是绝对 token 数而不是百分比，
50%/90% 并不落在进度条的对应位置。

压缩本身见 [ADR 0006](adr/0006-历史裁剪从自建改为框架在环压缩.md)，
口径见 [ADR 0009](adr/0009-占用口径分报告与有效，压缩分母扣掉固定开销.md)。

### 交接文档（Handoff）

**有效占用**到 0.8 时，让**当前模型**把这段对话写成一份「做完了什么 / 正在做什么 / 下一步」的
