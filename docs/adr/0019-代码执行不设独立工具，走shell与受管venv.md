...
6. **产出经 `file://` 进对话，不做捕获管道。** 图表写进 `AppPaths.Data.AgentOutputs`，
   ...
   **图片必须包一层链接**：`[![说明](file:///…)](file:///…)`。裸 `![](…)` 在 `LiveMarkdown`
   ...
   ~~包链接那一条已作废~~：核对所引版本（`LiveMarkdown.Avalonia` 2.4.0）的源码，
   裸图片的 `Link` 照样拿到 `HRef`（`LinkInlineNode`），按下/松开/激活整条链路都不区分
   图片与文字——引用格式简化为裸图 `![说明](file:///…)`，提示词里那段
   「外层方括号不是多余的」一并删除。当时写下那条所依据的大概是更早的版本。

7. **产出目录归 `Data` 而非 `Cache`。** 对话正文以链接引用它们，清掉就等于历史里留下一堆坏图；
   它们也不可重建——重跑一次是另一次推理。

8. **产出按会话分目录，目录名是 `{会话标题}_{会话id前8位}`，且改名不搬目录。**
   分会话是**正确性问题**：最初那版用一个扁平共享目录，两个会话各画一张 `chart.png`，
   后者会静默盖掉前者——旧对话回头看一切正常，只是图是错的。

   **改名不搬**是与 `FileMemoryLayout` 的关键分歧。那边的目录只被框架活状态引用，
   改名搬走无碍；这边的路径已经**逐字写进历史消息**，一搬就是一片打不开的图。
   代价明说：同一个会话改名后会多出一个目录，旧图留在旧目录里。清理靠 id 后缀通配
   （`AgentOutputLayout.DeleteAll`，挂在 `SessionManager.Delete` 上）。

   目录名进 `AgentAssemblyFacts`：提示词里逐字写着这个路径，改了标题不重建，
   模型就继续往旧目录写而用户在新目录里什么都看不到。

9. **venv 保持全局共享**（所有会话、所有角色一份）。按角色各建一份，装完科学栈是每份几百 MB。
   代价明说：**跨会话的包版本会互相顶**——A 会话装 pandas 2.x 会顶掉 B 会话依赖的 1.x。
   接受，因为"一个工具箱"本来就是开发机的常态。

   同理，没绑工作区时的工作目录 `Cache/Scratch` 也仍是全局共享的（先于本决策的既有行为）：
   两个会话都写 `plot.py` 会互相盖。它在 `Cache` 里、本就可弃，暂不改。
