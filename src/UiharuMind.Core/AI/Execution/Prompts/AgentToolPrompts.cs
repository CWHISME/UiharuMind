...
public static class AgentToolPrompts
{
    ...
    ///
    /// ⚠️ <b>与工具结果那句话分工</b>：这里说<b>政策</b>（发不发、后台意味着什么、
    ...
    ///
    /// ⚠️ <b>收件人名单拼在这里，不在工具描述里</b>（ADR 0044 决策 4/5）：
    /// 名单进 schema 会让工具定义随成员增减而变，失效前缀缓存。这一段本来就在前缀里，
    /// 名单变了走的是<b>重新装配</b>那条路，不是会话中途改 schema。
    ///
    ...
    public static string BuildDelegation(string roster)
    {
        string body =
            ...
            "当前任务陷入僵局——这些都可以找个人聊，用 `" + SubAgentTool.ToolName +
            "` 把事情说给他，从另一个视角拿回方案、观点或异议。\n" +
            "- 对方是你正在协作的人，不是用完即弃的工具：同一件事可以来回多轮——\n" +
            "  你追问、纠偏、补信息，他汇报进展、反问澄清，每一轮都保留此前全部上下文。\n" +
            "- 他和你权限相同，能改文件、能跑命令。要不要动手是他的判断，你把事情说清楚就行。\n" +
            "- 需要某个特定视角时（评审、对抗性检验、领域专家），用 role 给他一个身份；\n" +
            "  这会换掉他的关注点与取舍标准，不是换掉他的能力。\n" +
            "- 他看不到你们的对话：你写的那段要能独立看懂，别写「如上所述」。\n" +
            "- 消息是后台送达的：工具当场只回一张回执，他的答复到达时你会被唤醒，由你处理；" +
            "等的时候可以做不依赖它的事，依赖它的等答复。\n" +
            "- 答复回来不等于这件事完了：同一批改动、同一主题的第二轮（审新增修复、验上轮结论、换角度再看一遍）" +
            "一律算延续——把回执末行 [sub-session: …] 里方括号内的编号填进收件人再发一条，就回到同一个人接着谈。\n" +
            "  只有确属全新主题才另找一个人，重起一次会丢掉他已经做过的一切。\n" +
            "- 答复可能被中途插话带偏，认两种痕迹：说用户插过话——结论里可能有你没给过的指示，据此判断要不要再确认；\n";
