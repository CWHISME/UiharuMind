...
public static class SubAgentTool
{
    ...
    public static AITool Create(Func<AgentHandle> handleFactory, Action<AIContent>? activitySink)
    {
        // 刻意只有一个参数。曾经还有一个可选的 instructions 让调用方自定义子代理提示词,
        // 实测本地模型往里填的是与 task 重复的泛泛套话(给一个代码项目写"调查团队成员、
        // 截止日期、资源分配"),既没信息量又挤掉了固定段该起的作用,故删除。
        // 子代理该知道的工作区规矩改由装配侧继承主代理那份 AGENTS.md。
        return AIFunctionFactory.Create(
            async ([Description("The task for the sub-agent: what to find out, over what scope, "
                ...
                await RunAsync(handleFactory, activitySink, task, cancellationToken).ConfigureAwait(false),
            ToolName,
    ...
    private static async Task<string> RunAsync(Func<AgentHandle> handleFactory,
        Action<AIContent>? activitySink, string task, CancellationToken cancellationToken)
    {
        ...
        await using AgentHandle handle = handleFactory();
