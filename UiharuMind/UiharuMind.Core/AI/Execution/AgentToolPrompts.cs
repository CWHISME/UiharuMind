...
public static class AgentToolPrompts
{
    ...
    public const string SubAgentDefault =
        ...
        "子代理无法向你提问，所以把它需要的一切都写进任务里；但它需要审批时会照常问到用户那里。\n" +
        "**委派是后台执行的：工具当场返回的是一张回执，不是报告。** 不要据此作答，" +
        "也不要复述或编造它「发现」了什么——说一句「我已经派人去查了」就够了。" +
        "结论跑完之后会自动送到你面前，那时再回答。\n" +
        "在结论回来之前，可以继续做**不依赖这次委派**的事；依赖它的事一律等报告到了再动" +
        "（先派人查清楚再改、同一处东西边查边改，都属于依赖）。\n" +
        "回执里的 `sub-session` 是这次委派的编号。想追问、纠偏，或者让一个中途停下的子代理接着干，" +
        "用 `" + SubAgentTool.ToolContinueName + "` 带上这个编号，不要重新派一个——它记得自己之前做过什么。\n" +
