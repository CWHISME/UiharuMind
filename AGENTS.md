...
**必须从本文件所在目录（git 仓库根）启动 Claude Code。** 上一层 `Studys/UiharuMind/` 是本地工作壳，
里面不放任何 agent 文档；从那里启动会读不到本文件，**且不会报错**——症状是 agent 对本仓一无所知。
...
- **`DefaultCharacter` 枚举的成员名会被持久化成 `CharacterId`**，写进会话存档。改名会让已存会话
  找不到角色，症状是「角色丢了」，实机上很难归因。重构时枚举名一律不动。
- **`agentstate.json`**（todos / mode / 审批）按设计**可丢弃**，所以复制会话不带它是正常的；
  但**清空历史必须一并删除**，否则下次挂接会把旧任务清单读回来。
- 会话存档路径由 `SettingConfig` 的静态字段决定，无法重定向，所以碰存储的行为很难写单测。
